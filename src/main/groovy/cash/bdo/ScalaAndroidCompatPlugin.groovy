...
//        configureScaladoc(project, mainSourceSet, mainScalaCompile)
    ...
    private boolean resolveScalaSrcDirsToAndroidSourceSetsClosure(Project project, sourceSet, boolean evaluated) {
        ...
        if (verMajor >= 8) {
            final scalaSourceSet = factory.newInstance(DefaultScalaSourceSet, sourceSet.name, factory)
            sourceSet.convention.plugins.put('scala', scalaSourceSet)
            scalaDirSet = scalaSourceSet.scala

            // TODO: 如果上面的不能用了，就启用下面的。
//            final ScalaSourceDirectorySet scala = factory.newInstance(DefaultScalaSourceDirectorySet, factory.sourceDirectorySet('scala', "${sourceSet.name} Scala source"), dependencyFactory)
...
//            scalaDirSet = scala
            ...
            final scalaSourceSet = new DefaultScalaSourceSet(sourceSet.name, factory) {}
//            final scalaSourceSet = factory.newInstance(DefaultScalaSourceSet, sourceSet.name, factory)
